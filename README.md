<h1 align="center">Hi, I'm Fayyaz 👋</h1>
<h3 align="center">AI Engineer in the making | Agentic AI · Vibe Coding · LLM Systems</h3>
<p align="center">
  📍 Dubai, United Arab Emirates &nbsp;·&nbsp; 🌱 Currently deep in Agentic AI & AI Engineering training
</p>

🧠 About Me


🔭 I'm currently building projects around agentic AI systems — LLM-powered agents that can reason, plan, and use tools
🌱 Actively learning: Agentic AI, AI Engineering, and Vibe Coding (AI-assisted, fast-iteration development)
🧩 Interested in how LLMs, RAG, and multi-agent workflows come together to build real, usable products
💬 Ask me about: prompt engineering, LLM app workflows, or building with AI-first tools
📫 Reach me: [add your email or LinkedIn link here]

🛠️ Tech & Tools

<p align="left">
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat" />
  <img src="https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white" />
</p>


<p align="center"><i>Building in public as I go from learner to AI Engineer 🚀</i></p>
